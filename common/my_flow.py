...
def backwarp(tenInput, tenFlow):
    ...
    tenOutput = torch.nn.functional.grid_sample(input=tenInput, grid=(backwarp_tenGrid[str(tenFlow.size())] + tenFlow).permute(0, 2, 3, 1), mode='bilinear', padding_mode='zeros')

    tenMask = tenOutput[:, -1:, :, :]; tenMask[tenMask > 0.999] = 1.0; tenMask[tenMask < 1.0] = 0.0

    res = tenOutput[:, :-1, :, :] * tenMask
    return np.nan_to_num(res.transpose(1, 2).transpose(2, 3).numpy()[0])
...
def distortion(img, params=[]):
    ...
    if mode == 0: # Gaussian. params = [0, sigma]
        if pass_params:
            ...
            sigma = 1 + random.random() * 1
            params.append(sigma)
        #img = filters.gaussian(img, sigma=sigma)
        img = img * 255 # Img is in range [0, 1]
...
def split_frames(stereo=False):
    ...
    for i in range( 100, min(101, len(files_l)) ): # 3, 8 * 24 or 100, 101
